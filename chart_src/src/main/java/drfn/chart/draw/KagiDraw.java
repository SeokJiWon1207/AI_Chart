...
public class KagiDraw extends DrawTool{
    ...
    public KagiDraw(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm, cdm);
...
//	    int p = 0;
        ...
        double[] fColorInfo = new double[dataCnt];
        ...
        x = getBounds().left;
        y = calcy(fSwingData[0]);
        float y1;

        int[] kagiColor=CoSys.CHART_COLORS[0];
        double dPreStart = calcy(fSwingData[0]);

        if(fColorInfo[0]==1) {
            kagiColor = CoSys.CHART_COLORS[0];
        } else if(fColorInfo[0]==-1){
            kagiColor = CoSys.CHART_COLORS[1];
        }

        //중심선 그리기 옵션
        boolean bDrawCenterLine = true;
        int nCenterWidth = 3;
        ...
        for (int i=0; i<nPnfCount; i++) {
            ...
            int centerY = 0;
            if(y1<y) {//상승선
                centerY = ((int)y1 - (int)y) / 2;
                //전환선 색상으로 그리기
                if(y1<dPreStart) {
                    //Kagi 선 그리기
                    _cvm.drawLine(gl, (int)x,(int)dPreStart,(int)x,(int)y, kagiColor ,1.0f);

                    //색상변경
                    //2023.06.02 by SJW - 다크테마 차트 색상 변경 >>
//                    kagiColor = CoSys.CHART_COLORS[0];
                    if(_cvm.getSkinType() == COMUtil.SKIN_BLACK) {
                        kagiColor = CoSys.CHART_COLOR_UP_NIGHT;
                    } else {
                        kagiColor = CoSys.CHART_COLORS[0];
                    }
                    //2023.06.02 by SJW - 다크테마 차트 색상 변경 <<
                    _cvm.drawLine(gl, (int)(x),(int)dPreStart,(int)(x),(int)y1, kagiColor ,1.0f);
                } else {
                    //Kagi 선 그리기
                    _cvm.drawLine(gl, (int)x,(int)y,(int)x,(int)y1, kagiColor ,1.0f);
                }
            } else { //하락선  
                centerY = Math.abs(((int)y1 - (int)y) / 2);
                //전환선 색상으로 그리기
                if(y1>dPreStart) {
                    //Kagi 선 그리기
                    _cvm.drawLine(gl, (int)x,(int)dPreStart,(int)x,(int)y, kagiColor ,1.0f);
...
//                    kagiColor = CoSys.CHART_COLORS[1];
                    if(_cvm.getSkinType() == COMUtil.SKIN_BLACK) {
                        kagiColor = CoSys.CHART_COLOR_DOWN_NIGHT;
                    } else {
                        kagiColor = CoSys.CHART_COLORS[1];
                    }
                    //2023.06.02 by SJW - 다크테마 차트 색상 변경 <<
            ...
            //조정선 그리기
            _cvm.drawLine(gl, (int)x,(int)y1,(int)(x+xw),(int)y1, kagiColor ,1.0f);

            //중심선 그리기
            if(bDrawCenterLine)
                _cvm.drawLine(gl, (int)x,(int)y+centerY,(int)x+nCenterWidth,(int)y+centerY, kagiColor ,1.0f);
        ...
        _cvm.drawFillRect(gl, getBounds().right-(int)COMUtil.getPixel(75),(int)COMUtil.getPixel(03)+_cvm.Margin_T,(int)COMUtil.getPixel(75),viewH, CoSys.DKGRAY, 0.5f);
