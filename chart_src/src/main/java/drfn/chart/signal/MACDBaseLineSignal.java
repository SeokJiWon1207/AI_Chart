...
public class MACDBaseLineSignal extends AbstractGraph{
    //    int[] interval={12,26,9};
    int sub_margin;
    int[][] data;
    double[] macd;
    double[] signal;
    public MACDBaseLineSignal(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        String[] datakind = {"종가"};
        _dataKind = datakind;
//        m_strDefinitionHtml = "MACD.html";	//2015. 1. 13 각 보조지표 설명/활용법 추가(상세설정창)
    ...
    public String getName(){
        ...
        if(m_nStrategyType == 1)
            return "MACDBaseLine강세약세";
        else
            return "MACDBaseLine신호";
    }
}