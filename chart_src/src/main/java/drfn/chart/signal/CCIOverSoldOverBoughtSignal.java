...
public class CCIOverSoldOverBoughtSignal extends AbstractGraph{
    int[][] data;
    double[] cci;
    double[] signal;
    //int[] base=null;
    public CCIOverSoldOverBoughtSignal(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
...
//        int[] d = makeAverage(m_ma,interval[0]);
    ...
    public void drawGraph(Canvas g){
        drawStrategyBaseGraph(g,tool,100.0,-100.0);
    }
    public void drawGraph_withSellPoint(Canvas g){
    }
    public String getName(){
        if(m_nStrategyType == 1)
            return "CCIOverSoldOverBought강세약세";
        else
            return "CCIOverSoldOverBought신호";
    }
}