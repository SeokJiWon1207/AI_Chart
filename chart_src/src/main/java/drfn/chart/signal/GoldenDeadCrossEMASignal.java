 ...
 * 가격 이동평균 그래프
 */
public class GoldenDeadCrossEMASignal extends AbstractGraph{
	int[][] data;
	String[] datakind = {"종가"};
	int m_dataCnt;
	public GoldenDeadCrossEMASignal(ChartViewModel cvm, ChartDataModel cdm){
		super(cvm,cdm);
...
//		m_strDefinitionHtml = "price_movingaverage.html";	//2015. 1. 13 각 보조지표 설명/활용법 추가(상세설정창)
	...
	public void FormulateData(){
		if(!formulated){
			...
			if(closeData != null){
				...
				double[] moveAverageData = exponentialAverage(closeData, interval[0]);
				double[] moveAverageData2 = exponentialAverage(closeData, interval[1]);
	...
	public String getName(){
		if(m_nStrategyType == 1)
			return "GoldenDeadCrossEMA강세약세";
		else
			return "GoldenDeadCrossEMA신호";
	}
}