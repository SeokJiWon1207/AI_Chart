...
public class ROCStochSlowGraph extends AbstractGraph {
    ...
    public ROCStochSlowGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        ...
        m_strDefinitionHtml = "roc+stochastic.html";    //2018.05.02 by lyj 보조지표 설명/활용법 추가(상세설정창)
    }
    ...
    public void FormulateData() {
        ...
        double[] roc1 = new double[dLen];

        for(int i=interval[0]; i<dLen; i++){
            roc1[i] = this.ROC(i, 12, closeData);
        }
        //ROC+Stochastic = ROC 구한 후 Stochastic %K 에 대입할 때 종가 대신 ROC값을 넣음
        for(int i=1;i<dLen;i++){
        	double l= MinMax.getRangeMin(roc1,i+1,interval[1]);
            double h= MinMax.getRangeMax(roc1,i+1,interval[1]);
            if(h==l)
            	stoch_k[i] = 0;
            else
            	stoch_k[i]= ((roc1[i]-l)*100)/(h-l);
        }
...
//        }
    ...
    private double ROC(int lIndex, int lPeriod, double[] closeData) {
