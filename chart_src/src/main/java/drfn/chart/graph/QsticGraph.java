...
public class QsticGraph extends AbstractGraph{
	double[] graphData;
	double[] signal;
    int[][] data;//계산 전 데이터
    Vector<DrawTool> tool;
    public QsticGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        tool = getDrawTool();//드로우 툴을 구한다
        definition="Force Index";

        m_strDefinitionHtml = "qstic.html";    //2018.05.02 by lyj 보조지표 설명/활용법 추가(상세설정창)
    }
    //========================================
    //DEMA : 
    //========================================
    public void FormulateData(){
        graphData = getQstic(interval[0]);
...
//        _cdm.setPacketFormat(dt.getPacketTitle(), "× 0.01");
    ...
    private double[] getQstic(int interval) //signal 포함
    {
        ...
        double[] qstic = new double[dLen];
        for(int i = 0;i<dLen;i++){
            //qstic = n일간의 종가와 시가 차의 평균
            //signal = qsticd의 9일 이동평균
            qstic[i] = (closeData[i] - openData[i]);
        }
        qstic = makeAverage(qstic, interval);
//        double[] openAverage = makeAverage(openData, interval);
//        for(int i = 0;i<dLen;i++){
//            //qstic = n일간의 종가와 시가 차의 평균
//            //signal = qsticd의 9일 이동평균
//            qstic[i] = (closeAverage[i] - openAverage[i])/interval;
//        }
        return qstic;
    }
