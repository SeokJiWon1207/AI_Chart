...
public class MAOGraph extends AbstractGraph{
    int[][] data;
    double[] mao;
    double[] signal;
    //int[] base=null;
    public MAOGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        String[] datakind = {"종가"};
        _dataKind = datakind;
        ...
        m_strDefinitionHtml = "mao.html";	//2015. 1. 13 각 보조지표 설명/활용법 추가(상세설정창)
    }
    public void FormulateData(){
        double[] price = _cdm.getSubPacketData("종가");
        if(price==null) return;
        double[] shortAve = makeAverage(price, interval[0]);
        double[] longAve = makeAverage(price, interval[1]);
