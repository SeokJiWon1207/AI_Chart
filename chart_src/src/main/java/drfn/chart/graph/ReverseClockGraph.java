...
//	int nInterval=25; //25
	...
	public ReverseClockGraph(ChartViewModel cvm , ChartDataModel cdm){
		super(cvm,cdm);
		tool = getDrawTool();
	...
	public void FormulateData(){
		...
		_cdm.setSubPacketData(dt.getPacketTitle(),trade);
		
		String[] sEndData = new String[end.length];
		for(int i=0; i<end.length; i++) {
			sEndData[i] = String.valueOf(end[i]);
		}
		_cdm.setSubPacketData(dt.getPacketTitle()+"_거래량스트링", sEndData);
		_cdm.setSubPacketData(dt.getPacketTitle()+"_거래량", end);
		formulated=true;
	}
