...
public class StochSlowGraph extends AbstractGraph{
    ...
    int[] slow_k;
    int[] slow_d;
    public StochSlowGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
...
//        double[] stoch_d = exponentialAverage(stoch_k,interval[1], interval[0]-1);
//        double[] slow_d = exponentialAverage(stoch_d,interval[2],interval[0]+interval[1]-2);
        double[] stoch_d = exponentialAverage_DI(stoch_k,interval[1], interval[0]-1);
        double[] slow_d = exponentialAverage_DI(stoch_d,interval[2],interval[0]-1);
...
//        }
        ...
        if(isSellingSignalShow)
            t.drawSignal(gl, t_data, t1_data);
        //2014. 9. 15 매매 신호 보기 기능 추가<<
    }
