...
//    public int dataTypeBollingerband;
    ...
    public BandBGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        tool = getDrawTool();//드로우 툴을 구한다
        definition="BandB";
//        dataTypeBollingerband = ChartViewModel.AVERAGE_DATA_HLO3;
        dataTypeBollingerband = ChartViewModel.AVERAGE_DATA_CLOSE;

        m_strDefinitionHtml = "band__b.html";    //2018.05.02 by lyj 보조지표 설명/활용법 추가(상세설정창)
    }
    //========================================
    //DEMA : 
    //========================================
...
//    	graphData = getForceIndex(interval[0]);
        graphData = getBandB(interval[0], interval[1]);
...
//        _cdm.setPacketFormat(dt.getPacketTitle(), "× 0.01");
	    signal = exponentialAverage(graphData,interval[2]);
    ...
    public double[] getStandardDeviation(double[] data, double[] average, int interval,int col) {
        ...
        for(int i = interval -1 ; i<stDevia.length ; i++) {
            ...
            for(int j= i ; j>i-interval ; j--) {
                nDev += Math.pow( (data[j]-average[i]), 2);
            }
            deviation[i] = nDev;
