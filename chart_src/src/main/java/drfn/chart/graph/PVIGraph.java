...
public class PVIGraph extends AbstractGraph{
	double[] graphData;
	double[] signal;
    int[][] data;//계산 전 데이터
    Vector<DrawTool> tool;
    public PVIGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        tool = getDrawTool();//드로우 툴을 구한다
        definition="PVI";

        m_strDefinitionHtml = "pvi.html";    //2018.05.02 by lyj 보조지표 설명/활용법 추가(상세설정창)
    }
    //========================================
    //DEMA : 
    //========================================
    ...
    private double[] getPVI() //signal 포함
    {
        ...
        for(int i=0;i<dLen;i++){
            ...
            else
            {
                ...
                {
                    if(closeData[i-1]!=0)
                        pvi[i] = pvi[i-1] + ((closeData[i]-closeData[i-1])/closeData[i-1]) * 100;
                }
                else
                {
                    pvi[i] = pvi[i-1];
