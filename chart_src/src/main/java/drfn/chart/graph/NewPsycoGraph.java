...
public class NewPsycoGraph extends AbstractGraph{
    //int[] interval = {12,5,5};
    int[][] data;
    int[] slow_k;
    int[] slow_d;
    public NewPsycoGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        ...
        m_strDefinitionHtml = "newpsychology.html";	//2015. 1. 13 각 보조지표 설명/활용법 추가(상세설정창)
    }
    ...
    public void FormulateData() {
        ...
        double[] fUpRatio = new double[dLen];
        ...
        for(int i=1;i<dLen;i++){
           if(i<interval[0]){
            ...
            for(int j=i; j>i-interval[0]; j--){
                if(closeData[j-1] < closeData[j]){
                    upSize += closeData[j]-closeData[j-1];
                    upNum++;
                }
                //else{
                else if(closeData[j-1] > closeData[j]){
                    downSize += closeData[j-1] - closeData[j];
                    downNum++;
                }
            }
            upRate = upNum*upSize/(upSize+downSize)*100;
            downRate = downNum*downSize/(upSize+downSize)*100;

            if(upNum == 0) fUpRatio[i] = 0;
            else fUpRatio[i] = ((upRate-downRate)/interval[0]);
        }

        for(int i=0;i<tool.size();i++){
            DrawTool dt = (DrawTool)tool.elementAt(i);

                _cdm.setSubPacketData(dt.getPacketTitle(),fUpRatio);
