...
public class OBVStochSlowGraph extends AbstractGraph {
    //int[] interval = {12,5,5};
    int[][] data;
    int[] slow_k;
    int[] slow_d;
    Vector<DrawTool> tool;
    public OBVStochSlowGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        ...
        m_strDefinitionHtml = "obv+stochastic.html";    //2018.05.02 by lyj 보조지표 설명/활용법 추가(상세설정창)
    }
    ...
    public void FormulateData() {
        ...
        //OVB+Stochastic = OVB 구한 후 Stochastic %K 에 대입할 때 종가 대신 OVB값을 넣음
        for(int i=1;i<dLen;i++){
        	double l= MinMax.getRangeMin(obv,i+1,interval[0]);
            double h= MinMax.getRangeMax(obv,i+1,interval[0]);
            if(h==l)
            	stoch_k[i] = 0;
            else
            	stoch_k[i]= ((obv[i]-l)*100)/(h-l);
        }
