...
public class ROCGraph extends AbstractGraph{
    double[] roc1;
    double[] signal;
    int[][] data;
    //int[] base=null;

    public ROCGraph(ChartViewModel cvm, ChartDataModel cdm){
        super(cvm,cdm);
        String[] datakind = {"고가","저가","종가"};
        _dataKind = datakind;
        definition="이 지표는 Roc는 당일의 주가를 특정일의 주가로 나눈 것으로 추세의 속도를 측정한다.";
        m_strDefinitionHtml = "ROC.html";	//2015. 1. 13 각 보조지표 설명/활용법 추가(상세설정창)
    }
    /*
    ...
    public void FormulateData(){
        ...
        for(int i=interval[0]; i<dLen; i++){
            roc1[i] = this.ROC(i, interval[0], closeData);
        }
...
//    	ldRetVal = ( (ldClose - ldCloseOld) / ldCloseOld ) * 100.; //_MIRAE
        ldRetVal = (ldClose / ldCloseOld) * 100.;	//Daewoo
