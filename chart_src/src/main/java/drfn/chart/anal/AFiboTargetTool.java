...
public class AFiboTargetTool extends AnalTool {
	public AFiboTargetTool(Block ac) {
		super(ac);
		ncount = 3;
...
//		price = ChartUtil.getFormatedData(price, _ac._cdm.getPriceFormat());
//		_ac._cvm.drawString(g, at_col, x2 + x1 - x, (int) (y2 + y2 - y) - (int) COMUtil.getPixel(7), price);
	...
	private void drawLine(Canvas g, float x, float y, float x1, float y1, float x2, float y2) {
		...
		double[] dChg = new double[] {23.8, 38.2, 50, 61.8, 100, 123.8, 138.2, 150, 161.8, 200};
		_ac._cvm.drawLine(g, x, y, x1, y1, at_col, 1.0f);
		_ac._cvm.drawLine(g, x1, y1, x2, y2, at_col, 1.0f);
		_ac._cvm.drawLine(g, x2, y2, in.right, y2, at_col, 1.0f);
		double yPos;
		for(int i = 0 ; i < dChg.length; i++ ){
			yPos = (y2+(y1-y)*dChg[i]/100);
			String price = "" + getChartPrice((int) yPos);
			price = ChartUtil.getFormatedData(price, _ac._cdm.getPriceFormat(), _ac._cdm);
			_ac._cvm.drawLine(g, x2, (float)yPos, in.right, (float)yPos, at_col, 1.0f);
			_ac._cvm.drawString(g, at_col, x2, (int)yPos - (int) COMUtil.getPixel(7), price);
			if(i == dChg.length-1){
				_ac._cvm.drawLine(g, x2, y2, x2, (int)yPos, at_col, 1.0f);
			}
		}
