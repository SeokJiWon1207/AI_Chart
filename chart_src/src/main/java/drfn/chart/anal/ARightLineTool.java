...
public class ARightLineTool extends AnalTool{
   
    public ARightLineTool(Block ac){
        super(ac);
        ncount = 2;
...
//        int[] at_col1 = {0,0,0};
        ...
        if(x>x1){
            if(y>y1){
            	_ac._cvm.drawLine(g, x1,y1,x2,y-(int)((x2-x)*ratio1), at_col, 1.0f);
            }else{                                                                    
            	_ac._cvm.drawLine(g, x1,y1,x2,y+(int)((x2-x)*ratio1), at_col, 1.0f);
            }
        }else{
            if(y>y1){
                _ac._cvm.drawLine(g, x,y,x2,y-(int)((x2-x)*ratio), at_col, 1.0f);
            }else{
                _ac._cvm.drawLine(g, x,y,x2,y+(int)((x2-x)*ratio), at_col, 1.0f);
            }
        }
