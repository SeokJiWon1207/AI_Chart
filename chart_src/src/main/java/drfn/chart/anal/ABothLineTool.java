...
public class ABothLineTool extends AnalTool{

    public ABothLineTool(Block ac){
        super(ac);
        ncount = 2;
    ...
    public void draw(Canvas g){
        ...
        double ratio=ry/rx; //���� 
        double ratio1=(-1)*ry/rx; //  x>x1 �϶���  else�� �ȿ��ִ� ����� �ݴ�� ����Ͽ����Ѵ�.  2016.07.18
...
//        int[] at_col1 = {0,0,0};
        ...
        if(x>x1){
            if(y>y1){
            	_ac._cvm.drawLine(g, x2,y-(int)((x2-x)*ratio1),0,y-(int)((-x)*ratio1), at_col, 1.0f);
            }else{                                                                    
            	_ac._cvm.drawLine(g, x2,y+(int)((x2-x)*ratio1),0,y+(int)((-x)*ratio1), at_col, 1.0f);
            }
        }else{
            if(y>y1){
                _ac._cvm.drawLine(g, x2,y-(int)((x2-x)*ratio),0,y-(int)((-x)*ratio), at_col, 1.0f);
            }else{
                _ac._cvm.drawLine(g, x2,y+(int)((x2-x)*ratio),0,y+(int)((-x)*ratio), at_col, 1.0f);
            }
        }
