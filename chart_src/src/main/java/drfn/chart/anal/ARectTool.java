...
public class ARectTool extends AnalTool{

    public ARectTool(Block ac){
        super(ac);
        ncount = 2;
...
//        g.save();
//        g.clipRect(in.left ,COMUtil.currentFrame.height()-(in.top+in.height()), in.width(), in.height());
        if(isSelect){
...
//            _ac._cvm.drawRect(g, x1-2-selectAreaWidth/2,y1-2-selectAreaWidth/2,selectAreaWidth,selectAreaWidth, CoSys.at_col);
    ...
    private void drawRect(Canvas gl, float x, float y, float x1, float y1){
        if(y>y1){
            if(x>x1){
                _ac._cvm.drawRect(gl,x1,y1,x-x1,y-y1, at_col);
            }else{
                _ac._cvm.drawRect(gl,x,y1,x1-x,y-y1, at_col);
            }
        }else{
            if(x>x1){
                _ac._cvm.drawRect(gl,x1,y,x-x1,y1-y, at_col);
            }else{
                _ac._cvm.drawRect(gl,x,y,x1-x,y1-y, at_col);
            }
        }
    }
