...
public class ACycleLinesTool extends AnalTool{

    public ACycleLinesTool(Block ac){
        super(ac);
        ncount = 2;
    ...
    public void draw(Canvas g){
        ...
        //양끝점표시.
//        COMUtil.drawFillRect(g, x-5, y-2, 5, 5, at_col, 1.0f);
        //       COMUtil.drawFillRect(g, x1-5, y1-2, 5, 5, at_col, 1.0f);

        //       drawSelectedPointData(g,(int)x,(int)y,xIndex,""+data[0].y);
        //      drawSelectedPointData(g,(int)x1,(int)y1,xIndex1,""+data[1].y);

        int nStart = _ac._cvm.getIndex();
        int nEnd = nStart + _ac._cvm.getViewNum();

        int nIndexGap = Math.abs(xIndex1-xIndex);
        if(nIndexGap >0)
        {
            for(int i=xIndex1;i<nEnd ;i+=nIndexGap){
                float xPos = getDateToX(i);
                _ac._cvm.drawLine(g, xPos, in.top, xPos , in.top+in.height(), at_col ,1.0f);
            }
            for(int i=xIndex;i>=nStart ;i-=nIndexGap){
                float xPos = getDateToX(i);
                _ac._cvm.drawLine(g, xPos, in.top, xPos , in.top+in.height(), at_col ,1.0f);
            }
        }
