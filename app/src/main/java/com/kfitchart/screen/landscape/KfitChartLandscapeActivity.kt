package com.kfitchart.screen.landscape
...
import com.kfitchart.KfitBaseActivity
...
class KfitChartLandscapeActivity : KfitBaseActivity() {
    ...
    private var chartFileName = CHART_FILE_NAME_STOCK_DOMESTIC
    private var tradeChartPeriodWeekFindIndex = 0

    // 2021.09.28 by lyk - kakaopay - 네비게이션 바 높이 체크 >>
    private var navH = 0
    private var preNavH = -1
    private var isFirstLoad = true
    // 2021.09.28 by lyk - kakaopay - 네비게이션 바 높이 체크 <<

    private var stateOfChartConfigLoad = false
    private var m_bMarketLive = false // 장마감 상태 전역변수
    ...
    companion object {
        ...
        fun orientationIntent(
            ...
            return Intent(context, KfitChartLandscapeActivity::class.java).apply {
...
//    var tradeTotalDataList : MutableList<KfitTradingListItemEntity> = listOf<KfitTradingListItemEntity>().toMutableList() //매매내역 다음키 처리를 위한 전체 데이터
    ...
    fun reqTradeData(state: Boolean) {
        ...
        val firstTradedDate = this@KfitChartLandscapeActivity.getMethod(reqFirstDate, "")
...
//        val chgRateStr = Math.abs(chgRate.toDouble()).toString().plus("%")
        val chgRateStr: String = if (chgRate.isNullOrEmpty()) {
            ...
            try {
                ...
                // 2024.06.19 by SJW - 가로모드 등락률 소수점 둘째자리까지 표기 >>
//                "$absChgRate%"
                "%.2f".format(absChgRate).plus("%")
                // 2024.06.19 by SJW - 가로모드 등락률 소수점 둘째자리까지 표기 <<
...
//            m_bClickCrossSetting = true
    ...
    private fun setLandscapeUI() {
        ...
        //2024.06.21 by SJW - 시스템 설정 화면 크게 설정 시 종목명 잘리던 현상 수정 >>
        val layoutRule = headerViewLayout.layoutParams as RelativeLayout.LayoutParams
        layoutRule.addRule(RelativeLayout.LEFT_OF, chartHeaderLayout.id)
        layoutRule.addRule(RelativeLayout.ALIGN_PARENT_LEFT) //2024.06.24 by SJW - 종목 영역 왼쪽 정렬
        headerViewLayout.layoutParams = layoutRule
        //2024.06.21 by SJW - 시스템 설정 화면 크게 설정 시 종목명 잘리던 현상 수정 <<
    ...
    fun move(target: LinearLayout, distance: Float, direction: Int) {
        ...
        animationSet.start()
        if (direction == 1) {
            this@KfitChartLandscapeActivity.direction = -1
        } else {
            this@KfitChartLandscapeActivity.direction = 1
        }
    }
...
//            divider.visibility = View.VISIBLE
    ...
    fun createChart(
        ...
        // 차트 데이터 호출 리스너
        this.chartViewKfit?.setListener(
